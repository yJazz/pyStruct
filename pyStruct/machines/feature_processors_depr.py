...
def process_pod_to_1D_Descriptors(sample: PodSample):
...
def get_pods_and_samples(workspace, theta_degs, normalize_y):
    ...
    for sample in range(pod_manager.N_samples):
        ...
        for theta_deg in theta_degs:
            ...
            s = PodSample(bc, theta_deg, loc_index, T_wall)
...
class PodCoherentStrength(FeatureProcessor):
    ...
    def _collect_samples(self) ->  list[PodSample]:
    ...
    def get_structure_training_pairs(self) -> tuple[np.ndarray, np.ndarray]:
        """ 
        X: boundary conditions (x_labels), shape (N_samples, len(x_labels))
        y: features (y_labels), shape (N_samples, N_modes, len(y_labels))
        """
        samples = self.training_samples
        N_modes = self.feature_config.N_modes
        x_labels = self.feature_config.x_labels
        y_labels = self.feature_config.y_labels

        X = np.zeros((len(samples), len(x_labels)))
        y = np.zeros((len(samples), N_modes, len(y_labels)))

        for i, sample in enumerate(samples):
             y[i, ...] = process_pod_to_1D_Descriptors(sample)
             X[i, :] = sample.bc.array(x_labels)
